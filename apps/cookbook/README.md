...
- `/recipes/:id` — Edit recipe (MDXEditor for `courseMaterial`).
- `/recipes/:id/transcripts` — Read-only transcripts viewer.
- `/billing` — Stripe checkout / billing portal entry points.

## Env vars

See `.env.example`. `VITE_STUDENT_APP_ORIGIN` is used to build the live URL
that's written to a recipe on Publish and copied to the teacher's clipboard.
